...
However, there is now a renewed effort to reintroduce PSK authentication, making this draft an update to the {RFC9052}.
...
{: #fig-overview title="Overview of message flow of Variant 1." artwork-align="center"}
...
This variant incurs minimal modifications with respect to the current methods described in {RFC9052} and the fourth message remains optional.
...
{: #fig-overview title="Overview of message flow of Variant 2." artwork-align="center"}
...
# Message formatting and processing. Differences with respect to {RFC9052}#

This section specifies the differences on the message formatting compared to {RFC9052}.
...
The Initiator includes ID_CRED_PSK in message_1 and encodes the full message as a sequence of CBOR-encoded data items as specified in Section 5.2.1. of {RFC9052}

The Responder SHALL process message_1 as follows:

- Decode message_1.
- Retrieve the PSK using ID_CRED_PSK.
- Process message_1 as specified in Section 5.2.3. of {RFC9052}.
...
- CIPHERTEXT_3 is a concatenation of two different ciphertexts:
  ...
  - CIPHERTEXT_3_2 is a COSE_Encrypt0 object as defined in Sections 5.2 and 5.3 of {RFC9052}, with the EDHOC AEAD algorithm of the selected cipher suite, using the encryption key K_3, the initialization vector IV_3 (if used by the AEAD algorithm), the parameters described in Section 5.2 of {RFC9052} and the plaintext PLAINTEXT_3_2 described as follows:
